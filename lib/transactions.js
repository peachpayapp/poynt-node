...
/**
 * Captures a single transaction at a business.
 * @param {String} options.businessId
 * @param {String} options.transactionId
 * @param {String} options.requestId idempotency id. Retries should always attach request id
 ...
 * @return {Transaction} transaction
 */
module.exports.captureTransaction = function captureTransaction(options, next) {
  var hasErr = helpers.hasKeys(options, ["businessId", "transactionId"]);
  if (hasErr) {
    return next(hasErr);
  }
  ...
  this.request(
    ...
    function (err, transaction) {
      ...
      if (!transaction || !(transaction.status === "CAPTURED" || transaction.status === "PARTIALLY_CAPTURED")) {
        return next(new Error("Capture failed"));
      }
      next(null, transaction);
    }
  );
